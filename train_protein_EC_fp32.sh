...
[ -z "${warmup_steps}" ] && warmup_steps=5000
...
[ -z "${use_asl}" ] && use_asl=1
[ -z "${gamma_neg}" ] && gamma_neg=4
[ -z "${cls_type}" ] && cls_type=EC
[ -z "${esm_dim}" ] && esm_dim=1280
[ -z "${mask_side_chain}" ] && mask_side_chain=0
[ -z "${residue_only_ratio}" ] && residue_only_ratio=0
[ -z "${span_mask}" ] && span_mask=0
[ -z "${residue_only}" ] && residue_only=0
[ -z "${use_rope}" ] && use_rope=0
[ -z "${use_edge_attn}" ] && use_edge_attn=0
[ -z "${v_attn}" ] && v_attn=0
[ -z "${ft_esm}" ] && ft_esm=0
[ -z "${more_cls}" ] && more_cls=0
# [ -z "${checkpoint_name}" ] && checkpoint_name=checkpoint_last.pt
...
hyperparams=$name-$cls_type-prob$res_pos_mask_prob-scale$res_noise_scale-layers$num_layers-bsz$batch_size-update$update_freq-lr$lr-node_dim$node_dim-bsz$((batch_size*n_gpu*OMPI_COMM_WORLD_SIZE*update_freq))
...
mkdir ./store/dataset/spearmanr/$hyperparams
...
torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT --nnodes=$OMPI_COMM_WORLD_SIZE --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_IP  \
       $(which unicore-train) $data_path --user-dir ./vabs --train-subset train --valid-subset valid,test --best-checkpoint-metric f1 \
       --num-workers 8 --ddp-backend=c10d \
       --task protein_EC --loss protein_pocket_ec --arch protein_pocket_base  \
       --tensorboard-logdir $save_dir/tsb \
       --log-interval 10 --log-format simple \
       --save-interval-updates 500 --validate-interval-updates 500 --keep-interval-updates 1 --no-epoch-checkpoints  \

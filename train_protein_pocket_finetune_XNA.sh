...
[ -z "${OM_COMM_WORLD_SIZE}" ] && OMPI_COMM_WORLD_SIZE=1
...
[ -z "${esm_stack}" ] && esm_stack=0
[ -z "${atom_feat}" ] && atom_feat=0
[ -z "${edge_mask_prob}" ] && edge_mask_prob=0
...
torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT --nnodes=$OMPI_COMM_WORLD_SIZE --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_IP  \
       $(which unicore-train) $data_path --user-dir ./vabs --train-subset train --valid-subset valid,test --best-checkpoint-metric auc_vote \
       --maximize-best-checkpoint-metric \
       --num-workers 8 --ddp-backend=c10d \
       --task protein_pocket_ft_XNA --loss protein_pocket_ft --arch protein_pocket_base  \
       ...
       --esm-stack $esm_stack \
       --edge-mask-prob $edge_mask_prob \
       --restore-file ./store/$6/checkpoint_best.pt \

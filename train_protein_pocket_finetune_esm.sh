...
hyperparams=$name-cut$cutoff-esm$use_esm_feat-layers$num_layers-bsz$batch_size-update$update_freq-node_dim$node_dim-bsz$((batch_size*n_gpu*OMPI_COMM_WORLD_SIZE*update_freq))
...
torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT --nnodes=$OMPI_COMM_WORLD_SIZE --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_IP  \
       ...
       --task protein_pocket_ft_esm --loss protein_pocket_ft_esm --arch ESM_pocket  \

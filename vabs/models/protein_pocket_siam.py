...
@register_model("siamdiff")
class siamdiff_pocket_Encoder(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args):
        ...
        if self.args.siam_type:
            edge_layers = nn.ModuleList([SpatialEdge(radius=4.5, min_distance=0)])

            self.graph_construct = GraphConstruction(edge_layers=edge_layers,
        ...
        if self.args.siam_type == 1:
            loaded_params = torch.load('./store/siamdiff_gearnet_atom.pth')
        elif self.args.siam_type == 2:
            loaded_params = torch.load('./store/siamdiff_gearnet_res.pth')
        ...
        if self.args.siam_type == 1:
            self.pocket_cls_all = CLSHead(input_dim=128*6, output_cls=2, preln=args.preln)
        else:
            self.pocket_cls_all = CLSHead(input_dim=512 * 6, output_cls=538, preln=args.preln)
    ...
    def forward(
        ...
        if not self.args.siam_type:
            ...
            pocket_pred = self.pocket_cls_all(graph_feat)

        if self.args.inference:
            atom_nodes = node_feat[:, -128:]
            def output_res(atom_types, residue_ids, atom_features, num_residues):
                residue_features = torch.zeros(num_residues, 37, 128).type(self.dtype)
                print_all_shape(atom_types, residue_ids, atom_features, residue_features)
            ...
            return residue_features, residue_type
...
@register_model_architecture("siamdiff", "siamdiff")

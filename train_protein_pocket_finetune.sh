...
[ -z "${gb_feat}" ] && gb_feat=0
[ -z "${use_HMM}" ] && use_HMM=0
[ -z "${use_DSSP}" ] && use_DSSP=0
[ -z "${use_PSSM}" ] && use_PSSM=0
[ -z "${use_ss}" ] && use_ss=0
[ -z "${local_all}" ] && local_all=0
[ -z "${concat_style}" ] && concat_style=0
[ -z "${add_style}" ] && add_style=0
[ -z "${learnable_pos}" ] && learnable_pos=0
[ -z "${use_absolute}" ] && use_absolute=1
[ -z "${use_relative}" ] && use_relative=0
[ -z "${use_nerf_encoding}" ] && use_nerf_encoding=0
[ -z "${use_all_layers}" ] && use_all_layers=0
...
hyperparams=$name-cut$cutoff-ipa$use_ipa-esm$use_esm_feat-pg$use_pointgnn-cl$use_clean-layers$num_layers-bsz$batch_size-update$update_freq-node_dim$node_dim-bsz$((batch_size*n_gpu*OMPI_COMM_WORLD_SIZE*update_freq))
...
torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT --nnodes=$OMPI_COMM_WORLD_SIZE --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_IP  \
       $(which unicore-train) $data_path --user-dir ./vabs --train-subset train --valid-subset valid,COACH420,CHEN251,B277,ASTEX85,DT198 --best-checkpoint-metric loss \
       --maximize-best-checkpoint-metric \
       --num-workers 1 --ddp-backend=c10d \
       --task protein_pocket_ft --loss protein_pocket_ft --arch protein_pocket_base  \
       --tensorboard-logdir $save_dir/tsb \
       --fp16 --fp16-init-scale 4 --fp16-scale-window 256 \
       --log-interval 10 --log-format simple \
       --save-interval-updates 200 --validate-interval-updates 200 --keep-interval-updates 1 --no-epoch-checkpoints  \
